-- Final hardening:
-- 1) stale sync_token updates become no-ops
-- 2) CLAIMED requires claim ownership fields
-- 3) claim ownership consistency + CREATED external_event_id immutability

ALTER TABLE calendar_event_mappings
    ADD CONSTRAINT calendar_event_mappings_claimed_owner_check
    CHECK (
        status <> 'CLAIMED'
...
ALTER TABLE calendar_event_mappings
    VALIDATE CONSTRAINT calendar_event_mappings_claimed_owner_check;

CREATE OR REPLACE FUNCTION calendar_event_mappings_before_update()
RETURNS TRIGGER AS $$
BEGIN
    -- Stale write: ignore instead of erroring/retrying forever.
