...
public class SessionService {
    ...
    public void rescheduleSession(UUID sessionId, UUID hostId, Instant newStartTime) {
        rescheduleSession(sessionId, hostId, newStartTime, false, true);
    }

    @Transactional
    public void rescheduleSession(UUID sessionId, UUID hostId, Instant newStartTime,
                                  boolean acknowledgeExternalConflicts) {
        rescheduleSession(sessionId, hostId, newStartTime, acknowledgeExternalConflicts, true);
    }
     ...
     * @param keepOriginalTimeBlocked whether the vacated hour stays unavailable to the host's
     *        <em>other</em> event types. Defaults true on the shorter overloads: a host moves a
     *        session because they cannot make that time, so reopening it silently is the one
     *        outcome they never asked for. This does not control whether the moved occurrence is
     *        regenerated by its own rule — it never is, under either setting.
     */
    @Transactional
    public void rescheduleSession(UUID sessionId, UUID hostId, Instant newStartTime,
                                  boolean acknowledgeExternalConflicts,
                                  boolean keepOriginalTimeBlocked) {
        ...
        // Set on every move, not only the first: the choice belongs to the reschedule the host is
        // performing now. A session moved twice should honour the box as it was ticked the second
        // time, and re-blocking an hour the host previously released is the safe direction anyway.
        session.setOriginBlocksOtherEvents(keepOriginalTimeBlocked);
