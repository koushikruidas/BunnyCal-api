...
public interface IdempotencyKeyRepository extends JpaRepository<IdempotencyKey, UUID> {
    ...
    @Query("""
            UPDATE IdempotencyKey k
             ...
             WHERE k.userId = :userId AND k.route = :route AND k.key = :key
               AND k.status = com.daedalussystems.easySchedule.booking.idempotency.IdempotencyStatus.IN_PROGRESS
            """)
    ...
    @Query("""
            UPDATE IdempotencyKey k
               SET k.status = com.daedalussystems.easySchedule.booking.idempotency.IdempotencyStatus.FAILED,
                   k.responseStatus = 503,
                   k.responseBody = :abandonedBody,
                   k.completedAt = :now,
                   k.updatedAt = :now
             WHERE k.status = com.daedalussystems.easySchedule.booking.idempotency.IdempotencyStatus.IN_PROGRESS
